...
- [x] 3.5 Reference Huffman adapter (static, trained on same corpus)
- [x] 3.6 Snappy adapter (optional, skip if not found)
- [x] 3.7 OodleNetwork adapter (bench_oodle.c, compiled only if NETC_BENCH_WITH_OODLE=ON)
  ...
  - SDK path via CMake variable UE5_OODLE_SDK or env var NETC_OODLE_SDK_PATH
...
- [x] 4.2 Implement throughput benchmark (sustained MB/s over full corpus)
- [x] 4.3 Implement Mpps benchmark (1,000,000 packets per second measurement)
- [x] 4.4 Implement multi-core scaling benchmark (1, 2, 4, 8, 16 threads)
...
- [x] 6.3 Implement OODLE-* comparison gates (netc vs OodleNetwork, when --with-oodle is active)
- [x] 6.4 Implement baseline regression check (±5% warning, ±15% fail)
- [x] 6.5 Implement baseline save/load (bench/baselines/*.json)
- [x] 6.6 Exit code 0 on pass, 1 on fail

## 7. CLI and Build Integration
- [x] 7.1 Implement CLI argument parsing (--workload, --compressor, --count, --seed, --format, --output, --ci-check, --with-oodle, --oodle-sdk, --oodle-htbits, --oodle-gates, --mode, --baseline-dir, --save-baseline, --check-baseline)
- [x] 7.2 CMake target: bench (optional deps: zlib, lz4, zstd, snappy detected via pkg-config)
